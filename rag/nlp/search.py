...
class Dealer:
    def __init__(self, dataStore: DocStoreConnection):
        self.qryr = query.FulltextQueryer()  # 全文检索工具：处理关键词检索
        self.dataStore = dataStore           # 数据库/搜索引擎连接，对接底层数据库ES
    ...
    class SearchResult:
        ...
        aggregation: list | dict | None = None      # 文档统计
        keywords: list[str] | None = None           # 提取的关键词
        group_docs: list[list] | None = None        # ？

    async def get_vector(self, txt, emb_mdl, topk=10, similarity=0.1): # query embedding
    ...
    def get_filters(self, req): # 生成检索过滤条件
        condition = dict()
    ...
    def insert_citations(self, answer, chunks, chunk_v,
                         ...
                         chunk_meta=None): # 给大模型回答自动添加引用标注
