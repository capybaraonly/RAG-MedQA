...
async def extract_by_llm(tenant_id: str, tenant_llm_id: int, extract_conf: dict, memory_type: List[str], user_input: str,
    ...
    if tenant_llm_id:
        ...
        llm_config = get_model_config_by_type_and_name(LLMType.CHAT, llm_id)
    llm = LLMBundle(llm_config)
    if task_id:
...
async def embed_and_save(memory, message_list: list[dict], task_id: str=None):
    if memory.tenant_embd_id:
        ...
        embd_model_config = get_model_config_by_type_and_name(LLMType.EMBEDDING, memory.embd_id)
    embedding_model = LLMBundle(embd_model_config)
    if task_id:
...
def query_message(filter_dict: dict, params: dict):
    ...
    if memory.tenant_embd_id:
        ...
        embd_model_config = get_model_config_by_type_and_name(LLMType.EMBEDDING, memory.embd_id)
    embd_model = LLMBundle(embd_model_config)
