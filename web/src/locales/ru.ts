...
Пример: сообщение 1 КБ с эмбеддингом 1024 измерений — около 9 КБ. Лимит по умолчанию 5 МБ — примерно 500 таких сообщений.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Проведите тест извлечения, чтобы проверить, может ли RAG-MedQA восстановить целевой контент для LLM.',
      Parse: 'Парсинг',
      dataset: 'Датасет',
      ...
      uploadDescription:
        'Поддерживает одиночную или пакетную загрузку файлов. Для локально развернутого RAG-MedQA: общий ограничение размера файлов за одну загрузку составляет 1 ГБ, с ограничением пакетной загрузки в 32 файла. Нет ограничения на общее количество файлов на аккаунт. Для cloud.RAG-MedQA.io общий ограничение размера файлов за одну загрузку составляет 10 МБ, каждый файл не должен превышать 10 МБ, максимум 128 файлов на аккаунт.',
      chunk: 'Чанк',
      bulk: 'Пакетно',
      cancel: 'Отмена',
      close: 'Закрыть',
      ...
      rerankTip: `Опционально. Если оставить пустым, RAG-MedQA будет использовать комбинацию взвешенной схожести по ключевым словам и взвешенного косинусного сходства векторов; если выбрана rerank модель, взвешенный reranking счет заменит взвешенное косинусное сходство векторов. Имейте в виду, что использование rerank модели значительно увеличит время отклика системы. Если вы хотите использовать rerank модель, убедитесь, что вы используете SaaS reranker; если вы предпочитаете локально развернутую rerank модель, убедитесь, что вы запускаете RAG-MedQA с помощью docker-compose-gpu.yml.`,
      topK: 'Топ-K',
      ...
      html4excelTip: `Используется с методом чанкинга General. Когда отключено, электронные таблицы (XLSX или XLS (Excel 97-2003)) в базе знаний будут разобраны в пары ключ-значение. Когда включено, они будут разобраны в HTML таблицы, разделяя каждые 12 строк, если исходная таблица имеет более 12 строк. Подробности см. на https://RAG-MedQA.io/docs/dev/enable_excel2html.`,
      autoKeywords: 'Авто-ключевые слова',
      autoKeywordsTip: `Автоматически извлекает N ключевых слов для каждого чанка, чтобы повысить их рейтинг для запросов, содержащих эти ключевые слова. Имейте в виду, что дополнительные токены будут потребляться чат-моделью, указанной в 'Системных настройках модели'. Вы можете проверить или обновить добавленные ключевые слова для чанка из списка чанков. Подробности см. на https://RAG-MedQA.io/docs/dev/autokeyword_autoquestion.`,
      autoQuestions: 'Авто-вопросы',
      autoQuestionsTip: `Автоматически извлекает N вопросов для каждого чанка, чтобы повысить их рейтинг для запросов, содержащих эти вопросы. Вы можете проверить или обновить добавленные вопросы для чанка из списка чанков. Эта функция не нарушит процесс чанкинга в случае ошибки, за исключением того, что может добавить пустой результат в исходный чанк. Имейте в виду, что дополнительные токены будут потребляться LLM, указанной в 'Системных настройках модели'. Подробности см. на https://RAG-MedQA.io/docs/dev/autokeyword_autoquestion.`,
...
`,
      useRaptor: 'RAPTOR',
      useRaptorTip:
        'RAPTOR можно использовать для многошаговых вопросно-ответных задач. Перейдите на страницу Файлы, нажмите Сгенерировать > RAPTOR. Подробности: https://RAG-MedQA.io/docs/dev/enable_raptor',
      prompt: 'Промпт',
      promptTip:
...
Выше приведено содержимое, которое вам нужно суммировать.`,
      ...
      vietnamese: 'Вьетнамский',
      pageRank: 'Page rank',
      pageRankTip: `Вы можете назначить более высокий балл PageRank определённым базам знаний при извлечении. Соответствующий балл добавляется к комбинированным оценкам схожести извлечённых чанков, повышая их рейтинг. Подробности: https://RAG-MedQA.io/docs/dev/set_page_rank`,
      tagName: 'Тег',
      frequency: 'Частота',
      ...
      tagSet: 'Наборы тегов',
      tagSetTip: `
     <p> Выберите одну или несколько баз тегов для автоматического тегирования чанков. Подробности: https://RAG-MedQA.io/docs/dev/use_tag_sets</p>
...
<ul>
  ...
  <li>Авто-ключевые слова зависят от LLM и потребляют много токенов.</li>
      ...
      useGraphRagTip:
        'Строит граф знаний поверх чанков файлов текущей базы знаний для многошагового Q&A с вложенной логикой. Подробности: https://RAG-MedQA.io/docs/dev/construct_knowledge_graph',
...
**Основные правила:**
  ...
  - **Всегда учитывайте** всю историю диалога.`,
      ...
      variable: 'Переменная',
      variableTip: `Используется вместе с API управления чат-ассистентами RAG-MedQA, переменные помогают строить гибкие стратегии системных промптов. {knowledge} — зарезервированная переменная для чанков из указанных баз знаний; все переменные в фигурных скобках {} в «Системном промпте». Подробнее: https://RAG-MedQA.io/docs/dev/set_chat_variables`,
      add: 'Добавить',
      key: 'Ключ',
      ...
      betaError:
        'Пожалуйста, сначала получите RAG-MedQA API ключ на странице Системных настроек.',
      searching: 'Поиск...',
      parsing: 'Парсинг',
      uploading: 'Загрузка',
...
Пример: Virtual Hosted Style`,
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для каждой новой созданной базы знаний. Если вы не можете найти модель эмбеддингов в выпадающем списке, проверьте, используете ли вы облегченную версию RAG-MedQA (которая не включает модели эмбеддингов) или проверьте https://RAG-MedQA.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      img2txtModel: 'VLM',
      img2txtModelTip:
        'VLM по умолчанию для каждой новой созданной базы знаний. Она описывает изображение или видео. Если вы не можете найти модель в выпадающем списке, проверьте https://RAG-MedQA.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      ...
      rerankModel: 'Rerank',
      rerankModelTip: `Модель rerank по умолчанию для переранжирования чанков. Если вы не можете найти модель в выпадающем списке, проверьте https://RAG-MedQA.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.`,
      ttsModel: 'TTS',
      ttsModelTip:
        'Модель преобразования текста в речь по умолчанию. Если вы не можете найти модель в выпадающем списке, проверьте https://RAG-MedQA.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
