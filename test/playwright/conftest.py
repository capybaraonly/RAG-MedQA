...
def _rsa_encrypt_password(password: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Set RAG-MedQA_SEEDING_MODE=ui to skip API seeding."
        ) from exc
...
def base_url() -> str:
    value = os.getenv("RAG-MedQA_BASE_URL") or os.getenv("BASE_URL")
    if not value:
...
def seeded_user_credentials(base_url: str, login_url: str, browser) -> tuple[str, str]:
    ...
    seeding_mode = os.getenv("RAG-MedQA_SEEDING_MODE", "auto").strip().lower()
    if seeding_mode not in {"auto", "api", "ui"}:
        if _env_bool("PW_FIXTURE_DEBUG", False):
            print(
                f"[seeded] Unknown RAG-MedQA_SEEDING_MODE={seeding_mode!r}; using auto.",
                flush=True,
            )
        seeding_mode = "auto"
    ...
    if seeded_via is None and seeding_mode in {"auto", "ui"}:
        ...
        try:
            ...
            default_email = os.getenv("DEFAULT_SUPERUSER_EMAIL", "admin@RAG-MedQA.io")
            raise RuntimeError(
