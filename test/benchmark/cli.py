...
def _parse_args() -> argparse.Namespace:
    ...
    base_parser.add_argument(
        "--base-url",
        default=os.getenv("RAG-MedQA_BASE_URL") or os.getenv("HOST_ADDRESS"),
        help="Base URL (env: RAG-MedQA_BASE_URL or HOST_ADDRESS)",
    )
    base_parser.add_argument(
        "--api-version",
        default=os.getenv("RAG-MedQA_API_VERSION", "v1"),
    ...
    # Auth/login options
    base_parser.add_argument("--login-email", default=os.getenv("RAG-MedQA_EMAIL"), help="Login email")
    base_parser.add_argument("--login-nickname", default=os.getenv("RAG-MedQA_NICKNAME"), help="Nickname for registration")
    ...
    base_parser.add_argument("--llm-factory", default=os.getenv("RAG-MedQA_LLM_FACTORY"), help="LLM factory name")
    base_parser.add_argument("--llm-api-key", default=os.getenv("ZHIPU_AI_API_KEY"), help="LLM API key")
    base_parser.add_argument("--llm-api-base", default=os.getenv("RAG-MedQA_LLM_API_BASE"), help="LLM API base URL")
    base_parser.add_argument("--set-tenant-info", action="store_true", help="Set tenant default model IDs")
    base_parser.add_argument("--tenant-llm-id", default=os.getenv("RAG-MedQA_TENANT_LLM_ID"), help="Tenant chat model ID")
    base_parser.add_argument("--tenant-embd-id", default=os.getenv("RAG-MedQA_TENANT_EMBD_ID"), help="Tenant embedding model ID")
    base_parser.add_argument("--tenant-img2txt-id", default=os.getenv("RAG-MedQA_TENANT_IMG2TXT_ID"), help="Tenant image2text model ID")
    base_parser.add_argument("--tenant-asr-id", default=os.getenv("RAG-MedQA_TENANT_ASR_ID", ""), help="Tenant ASR model ID")
    base_parser.add_argument("--tenant-tts-id", default=os.getenv("RAG-MedQA_TENANT_TTS_ID"), help="Tenant TTS model ID")
    ...
    base_parser.add_argument("--dataset-name", default=os.getenv("RAG-MedQA_DATASET_NAME"), help="Dataset name when creating")
    ...
    parser = argparse.ArgumentParser(description="RAG-MedQA HTTP API benchmark", parents=[base_parser])
    ...
    chat_parser.add_argument("--chat-name", default=os.getenv("RAG-MedQA_CHAT_NAME"), help="Chat name when creating")
    chat_parser.add_argument("--chat-payload", help="Chat payload JSON or @file")
    chat_parser.add_argument("--model", default=os.getenv("RAG-MedQA_CHAT_MODEL"), help="Model name for OpenAI endpoint")
