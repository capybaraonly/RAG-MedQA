...
```
  PYTHONPATH=/Directory_name/RAG-MedQA/test uv run -m benchmark [global flags] <chat|retrieval> [command flags]
```
...
```
  --base-url
    Base server URL.
    Env: RAG-MedQA_BASE_URL or HOST_ADDRESS
  --api-version
    API version string (default: v1).
    Env: RAG-MedQA_API_VERSION
  --api-key
...
```
  PYTHONPATH=./test uv run -m benchmark chat \
    ...
    --message "What is the purpose of RAG-MedQA?" \
...
Defaults used:
  ...
  - Chat message: "What is the purpose of RAG-MedQA?"
