...
def _create_and_upload_in_batches(auth, dataset_id, num_docs, tmp_path, batch_size=100):
    ...
    for batch_start in range(0, num_docs, batch_size):
        ...
        for i in range(batch_start, batch_end):
            fp = tmp_path / f"RAG-MedQA_test_upload_{i}.txt"
...
class TestMetadataBatchUpdate:
    def test_batch_update_metadata(self, HttpApiAuth, add_dataset, RAG-MedQA_tmp_dir):
        """
        ...
        document_ids = _create_and_upload_in_batches(HttpApiAuth, dataset_id, 1010, RAG-MedQA_tmp_dir)
