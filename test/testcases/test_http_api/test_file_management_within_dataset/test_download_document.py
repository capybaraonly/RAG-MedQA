...
class TestAuthorization:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                RAG-MedQAHttpApiAuth(INVALID_API_TOKEN),
                109,
    ...
    def test_invalid_auth(self, invalid_auth, tmp_path, expected_code, expected_message):
        res = download_document(invalid_auth, "dataset_id", "document_id", tmp_path / "RAG-MedQA_tes.txt")
        assert res.status_code == 401
        with (tmp_path / "RAG-MedQA_tes.txt").open("r") as f:
...
def test_file_type_validation(HttpApiAuth, add_dataset, generate_test_files, request):
    ...
    res = download_document(
        ...
        dataset_id,
        document_id,
        fp.with_stem("RAG-MedQA_test_download"),
    )
...
class TestDocumentDownload:
    ...
    def test_invalid_document_id(self, HttpApiAuth, add_documents, tmp_path, document_id, expected_code, expected_message):
        ...
        res = download_document(
            ...
            dataset_id,
            document_id,
            tmp_path / "RAG-MedQA_test_download_1.txt",
        )
        assert res.status_code == codes.ok
        with (tmp_path / "RAG-MedQA_test_download_1.txt").open("r") as f:
            response_json = json.load(f)
        assert response_json["code"] == expected_code
        assert response_json["message"] == expected_message
    ...
    @pytest.mark.p3
    def test_same_file_repeat(self, HttpApiAuth, add_documents, tmp_path, RAG-MedQA_tmp_dir):
        num = 5
        ...
        for i in range(num):
            res = download_document(
                ...
                dataset_id,
                document_ids[0],
                tmp_path / f"RAG-MedQA_test_download_{i}.txt",
            )
            assert res.status_code == codes.ok
            assert compare_by_hash(
                RAG-MedQA_tmp_dir / "RAG-MedQA_test_upload_0.txt",
                tmp_path / f"RAG-MedQA_test_download_{i}.txt",
            )
...
def test_concurrent_download(HttpApiAuth, add_dataset, tmp_path):
    ...
    for i in range(count):
        assert compare_by_hash(
            tmp_path / f"RAG-MedQA_test_upload_{i}.txt",
            tmp_path / f"RAG-MedQA_test_download_{i}.txt",
        )
