...
def add_document_func(request, WebApiAuth, add_dataset, RAG-MedQA_tmp_dir):
    def cleanup():
...
def add_documents(request, WebApiAuth, add_dataset, RAG-MedQA_tmp_dir):
    def cleanup():
        res = list_documents(WebApiAuth, {"id": dataset_id})
        for doc in res["data"]["docs"]:
            delete_document(WebApiAuth, {"doc_id": doc["id"]})
    ...
    dataset_id = add_dataset
    return dataset_id, bulk_upload_documents(WebApiAuth, dataset_id, 5, RAG-MedQA_tmp_dir)


@pytest.fixture(scope="function")
def add_documents_func(request, WebApiAuth, add_dataset_func, RAG-MedQA_tmp_dir):
    def cleanup():
        res = list_documents(WebApiAuth, {"id": dataset_id})
        for doc in res["data"]["docs"]:
            delete_document(WebApiAuth, {"doc_id": doc["id"]})
    ...
    return dataset_id, bulk_upload_documents(WebApiAuth, dataset_id, 3, RAG-MedQA_tmp_dir)
...
def document_app_module(monkeypatch):
    ...
    deepdoc_excel_module.RAG-MedQAExcelParser = _StubExcelParser
    ...
    deepdoc_html_module.RAG-MedQAHtmlParser = _StubHtmlParser
